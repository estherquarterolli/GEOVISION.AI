...
3. No modo `visual`, execute a célula **Preparar a lista de imagens**. Para a coleção sem identificação dos imóveis, o notebook vem com `AVALIACAO_EXPLORATORIA=True` e cria `imagens.csv` automaticamente em `/content/drive/MyDrive/GeoVision-IA`. A coluna `edificio_id` fica vazia; não inventamos identidades. O inventário usa `01_imagens_brutas`; quando não há fotos nela, usa `02_dataset_limpo/train`, `val` e `test`. Arquivos CSV existentes são preservados. Para origem conhecida, desative a opção: o inventário cria `imagens.preencher.csv`, no qual você preenche os edifícios e salva como `imagens.csv`, separado por vírgulas. No modo `multimodal`, desative a opção e prepare `casos.csv` com dados de casos reais.
...
Este pacote entrega treinamento e funções experimentais. A API atual continua configurada para 224 × 224, `/255`, uma imagem por inferência e heurísticas de fusão. **O novo `.keras` não é substituição direta do `.h5` atual.** O contrato novo fica disponível, mas o carregador/rotas atuais ainda não o utilizam. Para integrar, conferir hash, schema 2, ordem das classes, quatro entradas multimodais, 600 × 660 RGB 0–255 e todas as categorias/medições; recusar contrato incompatível. Exportar simplesmente em `.h5` não resolve isso.
