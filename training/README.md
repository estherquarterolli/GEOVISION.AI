...
Para uma coleção grande no Drive, o notebook usa por padrão uma amostra determinística e balanceada de 300 imagens por classe no modo visual exploratório. Isso evita gastar horas apenas lendo/copiano arquivos antes da GPU e produz 60 imagens por classe no teste. O progresso de verificação, cópia e extração aparece a cada 25 imagens. Não defina `LIMITE_POR_CLASSE = 0` antes de conferir a primeira avaliação.
